...
class KGVisualService:
    ...
    def get_disease_subgraph(self, disease_name: str, top_n: int = 15) -> dict:
        ...
        return self._format_graph(nodes, links, f"疾病 '{disease_name}' 关联子图")
    ...
    def get_comorbidity_network(self, disease_name: Optional[str] = None, top_n: int = 20) -> dict:
        ...
        if disease_name:
            ...
            records = neo4j_client.run("""
                MATCH (d:Disease {name: $name})<-[:DIAGNOSED_WITH]-(v:Visit)-[:DIAGNOSED_WITH]->(co:Disease)
                WHERE co.name <> d.name
                RETURN id(d) AS did, id(co) AS coid, co.name AS co_name, count(DISTINCT v) AS cnt
                ORDER BY cnt DESC LIMIT $limit
            """, {"name": disease_name, "limit": top_n})
            ...
            for r in records:
                if center_id is None:
                    center_id = r["did"]
                    nodes[center_id] = {
                        "id": center_id, "label": "Disease", "name": disease_name,
